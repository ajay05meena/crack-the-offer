---
name: commit
description: Reviews and creates a git commit for this repo — refuses to commit directly on `main` (prompts to branch instead), inspects the actual staged/unstaged diff before staging anything, and updates `INDEX.md` when the diff adds or changes a problem/exercise/design doc that isn't reflected there yet. Use when the user asks to commit, "make a commit", "commit my changes", "commit this branch's work".
---

# Commit

Create a commit for this repo the way it should always be done here: never straight onto `main`, never blind about what's staged, and never leaving `INDEX.md` stale.
...
3. **Check `INDEX.md`.** Compare the changed paths against its tables:
   ...
   - A new or changed skill under `.claude/skills/` not listed in the AI skills table → add/update its row.

   If `INDEX.md` needs a change, edit it and fold it into the same commit — an index update is never a separate follow-up commit. If nothing's missing, say so in one line and move on.
